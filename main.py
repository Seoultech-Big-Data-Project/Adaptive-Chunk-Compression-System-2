...
#     s_ratio = s_orig / s_comp if s_orig > 0 else float("inf")
#     m_ratio = m_orig / m_comp if m_orig > 0 else float("inf")
...
#     results: {num_workers: {method: (orig, comp, time)}}
#     """
#     csv_path = Path("graph") / filename
...
#     """
#     워커 수가 4일 때의 결과로 Weighted Score 계산 후 CSV로 저장
#     results: {method: (orig, comp, time)}
#     """
#     csv_path = Path("graph") / filename
#     csv_path.parent.mkdir(exist_ok=True)
...
#         # ---------- 1. 모델 기반 ----------
#         print(f"\n>>> 1) 모델 기반 멀티 벤치마크 (워커 {num_workers}개)")
...
#         # ---------- 2. 단일 코덱들 ----------
#         for codec in ["zstd", "lz4", "snappy"]:
#             print(f"\n>>> 2) 단일 코덱 {codec} 멀티 벤치마크 (워커 {num_workers}개)")
...
#     # 워커 수 4일 때의 결과로 weighted score 계산
#     if 4 in all_results:
